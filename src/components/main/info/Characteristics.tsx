...
const Characteristics = () => {
	return (
		<div className="max-w-[636px] pl-12 border-l max-2xl:pl-8 max-xl:pl-0 max-xl:border-none max-xl:w-full max-xl:mt-8">
			...
			<ul className="flex justify-between flex-wrap gap-6 mt-8 max-xl:mt-6 max-sm:gap-5 max-xsm:gap-x-0">
				{characteristicsData.map((item) => (
					<li key={item.id} className="w-1/3 h-11 flex justify-between flex-col max-sm:w-[136px] max-xsm:w-1/2">
						<strong className="text-custom-black text-sm font-gotham-bold max-xsm:text-xs">{item.value}</strong>
